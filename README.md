...
# 任意の保存パスを設定したい場合は「save_path」を使用してください。デフォルトは実行されているプログラムと同じパス。
dictionary2 = Dictionary(name="使用例の辞書その2", save_path=Path("辞書￥カスタム"))
```
...
```py
dictionary = MicrosoftImeDictionary(name="辞書A")
```
